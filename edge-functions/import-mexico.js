...
var BOUNDS = {
  'Gasolina Regular': { min: 0.01, max: 2.50 },
  'Gasolina Premium': { min: 0.01, max: 2.50 },
  'Diésel Automotriz': { min: 0.001, max: 2.50 },
};

var MXN_TO_USD_FALLBACK = 0.058;
...
function validate(record) {
  ...
  if (precio < 0.01 || precio > 2.50) {
    return { valid: false, reason: 'price ' + precio.toFixed(4) + ' out of Mexico range [0.01, 2.50]' };
  }
...
async function batchUpsert(records) {
  ...
  for (var i = 0; i < records.length; i += BATCH_SIZE) {
    ...
    try {
      ...
      if (result.error) {
        console.error('[Mexico] Upsert error at offset ' + i + ': ' + JSON.stringify(result.error));
        allErrors.push({
      ...
      console.error('[Mexico] Upsert exception at offset ' + i + ': ' + err.message);
      allErrors.push({
...
async function writeIngestionLog(stats) {
  try {
    ...
    var result = await client.database
      ...
      .insert([logEntry]);
    if (result.error) {
      console.error('[Mexico] Failed to write _etl_runs: ' + JSON.stringify(result.error));
    } else {
      console.log('[Mexico] Ingestion log written');
    }
  } catch (err) {
    console.error('[Mexico] Log exception: ' + err.message);
  }
}

export default async function importMexico(req) {
  ...
  try {
    ...
    console.log('[Mexico] Processing ' + rawRecords.length + ' records in ' + mode + ' mode');

    var exchangeRate = body.exchangeRate || MXN_TO_USD_FALLBACK;

    var transformed = transformRecords(rawRecords, exchangeRate);
    stats.records_rejected = transformed.rejectedCount;
    console.log('[Mexico] Valid: ' + transformed.validCount + ', Rejected: ' + transformed.rejectedCount);
    ...
    stats.duration_seconds = Math.round((Date.now() - startTime) / 1000);
    console.error('[Mexico] Catastrophic failure: ' + err.message);
    await writeIngestionLog(stats);
    return new Response(
