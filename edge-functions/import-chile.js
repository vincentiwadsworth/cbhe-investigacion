...
async function batchUpsert(records) {
  ...
  for (var i = 0; i < records.length; i += BATCH_SIZE) {
    ...
    try {
      ...
      if (result.error) {
        console.error('[Chile] Upsert error at offset ' + i + ': ' + JSON.stringify(result.error));
        allErrors.push({
      ...
      console.error('[Chile] Upsert exception at offset ' + i + ': ' + err.message);
      allErrors.push({
...
async function writeIngestionLog(stats) {
  try {
    ...
    var result = await client.database
      ...
      .insert([logEntry]);
    if (result.error) {
      console.error('[Chile] Failed to write _etl_runs: ' + JSON.stringify(result.error));
    } else {
      console.log('[Chile] Ingestion log written');
    }
  } catch (err) {
    console.error('[Chile] Log exception: ' + err.message);
  }
}
...
export default async function importChile(req) {
  ...
  try {
    ...
    console.log('[Chile] Processing ' + rawRecords.length + ' records in ' + mode + ' mode');
    ...
    console.log('[Chile] Valid: ' + transformed.validCount + ', Rejected: ' + transformed.rejectedCount);
    ...
    stats.duration_seconds = Math.round((Date.now() - startTime) / 1000);
    console.error('[Chile] Catastrophic failure: ' + err.message);
    await writeIngestionLog(stats);
    return new Response(
