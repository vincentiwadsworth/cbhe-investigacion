...
var PAIS = 'Colombia';
var DATA_SOURCE = 'Colombia Socrata';
var FUENTE = 'datos.gov.co';
var MONEDA = 'COP';
var FRECUENCIA = 'mensual';
var BATCH_SIZE = 500;

var BOUNDS = {
  'Gasolina Corriente': { min: 0.01, max: 2.50 },
  'Gasolina Extra': { min: 0.01, max: 2.50 },
  'Diésel': { min: 0.001, max: 2.50 },
};

var COP_TO_USD_FALLBACK = 0.00024;
...
function validate(record) {
  ...
  if (precio < 0.01 || precio > 2.50) {
    return { valid: false, reason: 'price ' + precio.toFixed(4) + ' out of Colombia range [0.01, 2.50]' };
  }
...
async function batchUpsert(records) {
  ...
  for (var i = 0; i < records.length; i += BATCH_SIZE) {
    ...
    try {
      ...
      if (result.error) {
        console.error('[Colombia] Upsert error at offset ' + i + ': ' + JSON.stringify(result.error));
        allErrors.push({
      ...
      console.error('[Colombia] Upsert exception at offset ' + i + ': ' + err.message);
      allErrors.push({
...
async function writeIngestionLog(stats) {
  try {
    ...
    var result = await client.database
      ...
      .insert([logEntry]);
    if (result.error) {
      console.error('[Colombia] Failed to write _etl_runs: ' + JSON.stringify(result.error));
    } else {
      console.log('[Colombia] Ingestion log written');
    }
  } catch (err) {
    console.error('[Colombia] Log exception: ' + err.message);
  }
}

export default async function importColombia(req) {
  ...
  try {
    ...
    console.log('[Colombia] Processing ' + rawRecords.length + ' records in ' + mode + ' mode');

    var exchangeRate = body.exchangeRate || COP_TO_USD_FALLBACK;

    var transformed = transformRecords(rawRecords, exchangeRate);
    stats.records_rejected = transformed.rejectedCount;
    console.log('[Colombia] Valid: ' + transformed.validCount + ', Rejected: ' + transformed.rejectedCount);
    ...
    stats.duration_seconds = Math.round((Date.now() - startTime) / 1000);
    console.error('[Colombia] Catastrophic failure: ' + err.message);
    await writeIngestionLog(stats);
    return new Response(
