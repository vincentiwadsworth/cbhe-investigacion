...
var BOUNDS = {
  'Nafta Súper': { min: 0.01, max: 2.50 },
  'Nafta Premium': { min: 0.01, max: 2.50 },
  'Diésel': { min: 0.001, max: 2.50 },
  'GNC': { min: 0.001, max: 1.50 },
};
...
function validate(record) {
  ...
  if (!bounds) {
    if (precio < 0.001 || precio > 2.50) {
      return { valid: false, reason: 'price ' + precio.toFixed(4) + ' out of range' };
    }
  } else if (precio < bounds.min || precio > bounds.max) {
    return { valid: false, reason: 'price ' + precio.toFixed(4) + ' out of range' };
  }
...
async function batchUpsert(records) {
  ...
  for (var i = 0; i < records.length; i += BATCH_SIZE) {
    ...
    try {
      ...
      if (result.error) {
        console.error('[Argentina] Upsert error at offset ' + i + ': ' + JSON.stringify(result.error));
        allErrors.push({
      ...
      console.error('[Argentina] Upsert exception at offset ' + i + ': ' + err.message);
      allErrors.push({
...
async function writeIngestionLog(stats) {
  try {
    ...
    var result = await client.database
      ...
      .insert([logEntry]);
    if (result.error) {
      console.error('[Argentina] Failed to write _etl_runs: ' + JSON.stringify(result.error));
    } else {
      console.log('[Argentina] Ingestion log written');
    }
  } catch (err) {
    console.error('[Argentina] Log exception: ' + err.message);
  }
}
...
export default async function importArgentina(req) {
  ...
  try {
    ...
    console.log('[Argentina] Processing ' + rawRecords.length + ' records in ' + mode + ' mode');

    var exchangeRate = body.exchangeRate || ARS_TO_USD_FALLBACK;

    var transformed = transformRecords(rawRecords, exchangeRate);
    stats.records_rejected = transformed.rejectedCount;
    console.log('[Argentina] Valid: ' + transformed.validCount + ', Rejected: ' + transformed.rejectedCount);
    ...
    stats.duration_seconds = Math.round((Date.now() - startTime) / 1000);
    console.error('[Argentina] Catastrophic failure: ' + err.message);
    await writeIngestionLog(stats);
    return new Response(
