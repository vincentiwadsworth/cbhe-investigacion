...
export default async function importChile(req) {
  ...
  var client = createClient({ baseUrl: baseUrl, anonKey: anonKey });
  ...
  function transformRecords(rawRecords, exchangeRate) {
    ...
    for (var i = 0; i < rawRecords.length; i++) {
      ...
      var record = {
        fecha: raw.fecha, pais: PAIS, producto: producto,
        precio_usd: precioUsd, precio_local: precioLocal, moneda: MONEDA,
        fuente: FUENTE, data_source: DATA_SOURCE, frecuencia: FRECUENCIA,
        quality_score: raw.quality_score != null ? Number(raw.quality_score) : 1.0
      };
  ...
  async function batchUpsert(records) {
    ...
    for (var i = 0; i < records.length; i += BATCH_SIZE) {
      ...
      try {
        ...
        if (result.error) { allErrors.push({ error: String(result.error), batch_start: i }); }
  ...
  if (req.method !== 'POST') {
    return new Response(JSON.stringify({ error: 'Use POST with JSON body' }), { status: 405, headers: { 'Content-Type': 'application/json' } });
  }
  ...
  try {
    ...
    if (rawRecords.length === 0) {
      ...
      return new Response(JSON.stringify({ message: 'No records provided', stats: stats }), { status: 200, headers: { 'Content-Type': 'application/json' } });
    }
    ...
    if (transformed.records.length === 0) {
      stats.duration_seconds = Math.round((Date.now() - startTime) / 1000);
      return new Response(JSON.stringify({ message: 'No valid records', rejected_count: transformed.rejectedCount, stats: stats }), { status: 200, headers: { 'Content-Type': 'application/json' } });
    }
    ...
    try {
      ...
      await client.database.from('_etl_runs').insert([{
        source: 'Chile ' + DATA_SOURCE, run_type: stats.run_type,
        records_fetched: stats.records_total, records_inserted: stats.records_inserted,
        records_rejected: stats.records_rejected, error_count: stats.error_count,
        duration_seconds: stats.duration_seconds, quality_score: quality
      }]);
    } catch (logErr) { /* swallow */ }

    return new Response(JSON.stringify({
      message: 'import complete', mode: stats.run_type,
      records_total: stats.records_total, records_valid: transformed.validCount,
      records_inserted: stats.records_inserted, records_rejected: stats.records_rejected,
      errors: stats.error_count, duration_seconds: stats.duration_seconds
