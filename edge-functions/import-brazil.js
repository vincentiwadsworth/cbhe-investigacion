...
var BOUNDS = {
  'Gasolina': { min: 0.01, max: 2.50 },
  'Etanol': { min: 0.01, max: 2.00 },
  'Diesel S10': { min: 0.001, max: 2.50 },
  'GNV': { min: 0.001, max: 1.50 },
};

var BRL_TO_USD_FALLBACK = 0.18;
...
async function batchUpsert(records) {
  ...
  for (var i = 0; i < records.length; i += BATCH_SIZE) {
    ...
    try {
      ...
      if (result.error) {
        console.error('[Brazil] Upsert error at offset ' + i + ': ' + JSON.stringify(result.error));
        allErrors.push({
      ...
      console.error('[Brazil] Upsert exception at offset ' + i + ': ' + err.message);
      allErrors.push({
...
async function writeIngestionLog(stats) {
  try {
    ...
    var result = await client.database
      ...
      .insert([logEntry]);
    if (result.error) {
      console.error('[Brazil] Failed to write _etl_runs: ' + JSON.stringify(result.error));
    } else {
      console.log('[Brazil] Ingestion log written');
    }
  } catch (err) {
    console.error('[Brazil] Log exception: ' + err.message);
  }
}

export default async function importBrazil(req) {
  ...
  try {
    ...
    console.log('[Brazil] Processing ' + rawRecords.length + ' records in ' + mode + ' mode');

    var exchangeRate = body.exchangeRate || BRL_TO_USD_FALLBACK;

    var transformed = transformRecords(rawRecords, exchangeRate);
    stats.records_rejected = transformed.rejectedCount;
    console.log('[Brazil] Valid: ' + transformed.validCount + ', Rejected: ' + transformed.rejectedCount);
    ...
    stats.duration_seconds = Math.round((Date.now() - startTime) / 1000);
    console.error('[Brazil] Catastrophic failure: ' + err.message);
    await writeIngestionLog(stats);
    return new Response(
