// edge-functions/fetch-chile-api.ts
...
const CLP_TO_USD = 0.0012; // Tipo de cambio aproximado

export default async function fetchChileAPI(req: Request) {
  try {
    // Simulación de API - en producción reemplazar con endpoint real
    const mockData = [
      { producto: 'Gasolina 93', precio: 950 },
      { producto: 'Gasolina 97', precio: 1050 },
      { producto: 'Diésel', precio: 890 }
    ];
    ...
    const prices = mockData.map((item: any) => ({
      ...
      precio_usd: parseFloat(item.precio) * CLP_TO_USD,
      moneda: 'CLP',
      fuente: 'CNE'
    }));
    ...
    console.error('Error fetching Chile API:', error);
    return new Response(
